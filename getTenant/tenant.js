...
var tenant = {
    ...
    countData: function (arrData) {
        ...
        return { dataPercent: dataPercent, sumHouses: sumHousesTenant }
    },
    ...
    getHighHouse: function(arrData, price) {
        let billionHouse = arrData.map(element => {
    ...
    getRateHouse: function(arrData) {
        dataApp.get('/tenant/rate-house', (request, response) => {
            let result = [];
            if(request.query.data) {
                ...
                if(request.query.province) result = result.filter(element => this.getProvinceValue(arrData, request.query.province).includes(element));
                if(request.query.district) result = result.filter(element => this.getDictrictValue(arrData, request.query.district).includes(element));
    ...
    getValueByProvince: function(arrData) {
        dataApp.get('/tenant/value-search-province', (request, response) => {
            const init = this.getProvinceValue(arrData, request.query.province).filter(x => x);
            let countData;
            if(request.query.province) countData = this.countData(init);
            else countData = this.countData(arrData);
    ...
    getRateArea: function(arrData) {
        dataApp.get('/tenant/price-data', (request, response) => {
            const init = this.getProvinceValue(arrData, request.query.province).filter(x => x);
