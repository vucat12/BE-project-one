...
var seller = {
    ...
    getListPriceAndArea(arrData) {
        dataApp.get('/seller/list-price-area', (request, response) => {
            let result = [];
            result = arrData.filter(element => this.getProvinceValue(arrData, request.query.province).includes(element));
            result = result.filter(element => this.getDictrictValue(arrData, request.query.district).includes(element));

            let resData = result = result.map(element => {
                if (element.priceInf.trim().indexOf("tỷ") > 0) {
                    return [parseInt(element.areaInf.trim().slice(0, -3).replace(',', '.')),
                    parseInt(element.priceInf.trim().slice(0, -3).replace(',', '.')) * 1000]
                }
            }).filter(x => x);
